...
class Src(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if not isinstance(error, commands.CommandNotFound):
            await ctx.send(
                "Something went wrong! Make sure everything is formatted and spelled correctly."
            )
            print(error)
    ...
    @commands.command(name="gamesplayed", aliases=["gp"])
    async def gamesplayed(
        self,
        ctx,
        user=None,
    ):
        if user:
            if (user[0] + user[-1]) == "[]":
                for each in user[1:-1].split(","):
                    print(each)
                    await ctx.author.send(await srbot.gamecount(pformat(each)))
            else:
    ...
    async def invite(self, ctx):
        await ctx.send(
            "To add speedrun.bot to your server, click here:\n<"
            + oauth_url(self.bot.user.id)
            + ">"
        )

    @commands.command(
        name="source", aliases=["credit", "credits", "code", "sourcecode"]
    )
    async def source(self, ctx):
        await ctx.send(
            "The source code is available at https://github.com/Slush0Puppy/speedrunbot"
        )
    ...
    async def command_help(self, ctx, command=""):
        if command:
            await ctx.send(
                ctx.prefix + srbot.srbot_help[self.bot.get_command(command).name]
            )
        else:
